...
def all_points_core_distance(distance_matrix, d=2.0):
    """
    Compute the all-points-core-distance for all the points of a cluster
    without numerical overflow by using logarithms.
    ...
    # Compute s_{ij} = -d * log(d_{ij})
    s_ij = -d * np.log(dists)
    # Exclude self-distances by setting s_{ii} to -inf
    np.fill_diagonal(s_ij, -np.inf)
    # Compute log S_i for each point using logsumexp for numerical stability
    log_S_i = logsumexp(s_ij, axis=1)
    # Compute log m_i = log S_i - log(N - 1)
    log_m_i = log_S_i - np.log(N - 1)
    # Compute log of the all-points-core-distance
    log_apcd_i = - (1.0 / d) * log_m_i
    # Exponentiate to get the all-points-core-distance
