...
class DeepCluster:
    ...
    def perform_clustering(self):
        match self.clustering_method:
            ...
            case ClusterMethod.DBSCAN:
                    print("Performing DBSCAN cluseting...")
                    self.dbscan = DBSCAN(eps=15,min_samples=5)
                    self.dbscan_labels = self.dbscan.fit_predict(self.all_features)
    ...
    def evaluate_clustering(self):
        ...
        match self.clustering_method:
            ...
            case ClusterMethod.DBSCAN:
                self.nmi = normalized_mutual_info_score(self.all_labels, self.dbscan_labels)
                self.ari = adjusted_rand_score(self.all_labels, self.dbscan_labels)
    ...
    def visualize(self):
        ...
        plt.subplot(1, 2, 2)
        if self.clustering_method is ClusterMethod.DBSCAN:
            sns.scatterplot(x=tsne_features[:, 0], y=tsne_features[:, 1], hue=self.dbscan_labels, palette=sns.color_palette("hsv", len(set(self.dbscan_labels))), legend='full')
        else:
